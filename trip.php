<!--Test Oracle file for UBC CPSC304 2018 Winter Term 1
  ...
  <html>
    ...
    <head>
        ...
        <style>
    body {background-image: url('https://i.kym-cdn.com/photos/images/original/001/879/958/fb1.gif');
  ...
  background-size: 100% 100%;
    ...
    <body>
        ...
        <form method="GET" action="trip.php"> <!--refresh page when submitted-->
            ...
            <input type="submit" value="Aggregate" name="aggGroupBy"></p>
        </form>
        ...
        <form method="GET" action="trip.php"> <!--refresh page when submitted-->
            ...
            <input type="submit" value = "Having" name="aggHaving"></p>
        </form>

        <hr />
      
        <h3>  Find the average trip fee of each transport vehicle belonging to at least three trips </h3>
        <form method="GET" action="trip.php"> <!--refresh page when submitted-->
            <input type="hidden" id="nestedAggRequest" name="nestedAggRequest">
            <input type="submit" value = "nestedAgg" name="nestedAgg"></p>
        </form>

        <hr />

<!--         <h3> Find the origin airports that have every airplane model based in vancouver </h3>
        <form method="GET" action="trip.php"> refresh page when submitted
        ...
        function handleInsertRequest() {
            ...
            executeBoundSQL("insert into trip values (:bind1, :bind2, :bind3, :bind4, :bind5, :bind6, :bind7)", $alltuples);
            OCICommit($db_conn);
        }
        ...
        function handleDeleteRequest() {
            ...
            executePlainSQL("DELETE FROM trip WHERE tripid='" . $tripID . "'");
            OCICommit($db_conn);
        }
        ...
        function handleUpdateRequest() {
            ...
            executePlainSQL("UPDATE trip SET departuredate='" . $newDep . "', arrivaldate='" . $newArr . "' WHERE tripid='" . $tripID . "'");
        ...
        function handleSelectRequest() {
            ...
            $result = executePlainSQL("SELECT * FROM trip WHERE origin='" . $city . "'");

            echo "<br>Retrieved data from table trip:<br>";
            echo "<table>";
        ...
        function handleProjectRequest() {
            ...
            $result = executePlainSQL("SELECT tripid, departuredate, arrivaldate, fee FROM trip WHERE tripid='" . $tripID . "'");
        ...
        function handleJoinRequest() {
            ...
            $result = executePlainSQL("SELECT DISTINCT T.tripid, A.transid FROM Airplane A, trip T WHERE T.transid = A.transid AND T.origin='" . $city . "' AND T.departuredate='" . $dep . "' ");
        ...
        function handleGroupByRequest() {
            ...
            $result = executePlainSQL("SELECT destination, Min(fee) FROM trip GROUP BY destination");
        ...
        function handleHavingRequest() {
            ...
            $result = executePlainSQL("SELECT origin, Count(*) FROM trip GROUP BY origin HAVING Count(*) > 1 ");
        ...
        function handleNestedAggRequest() {
            ...
            $result = executePlainSQL("SELECT t.transid, AVG(t.fee)
                                        FROM trip t
                                        GROUP BY t.transid
                                        HAVING 2 < (SELECT Count(*) 
                                                    FROM trip t1
                                                    WHERE t1.transid = t.transid)");

            echo "<br>Listing by transportation id:<br>";
            echo "<table>";
            echo "<tr><th> transportation ID </th><th> Average fee </th></tr>";
        ...
        function handleDivisionRequest() {
            ...
            $result = executePlainSQL("SELECT distinct b.email 
                                       FROM bookby b
                                       WHERE NOT EXISTS (SELECT  b.tripid 
                                                           FROM  bookby b
                                                           EXCEPT
                                                           SELECT b1.tripid
                                                           FROM bookby b1
                                                           WHERE b1.tripid = b.tripid))" );

            echo "<br>Listing emails :<br>";
            echo "<table>";
        ...
        function handleDisplayTripRequest() {
            ...
            $result = executePlainSQL("SELECT * FROM Trip");
	...
	// A better coding practice is to have one method that reroutes your requests accordingly. It will make it easier to add/remove functionality.
		...
		if (isset($_POST['updateSubmit']) || isset($_POST['insertSubmit']) || isset($_POST['deleteSubmit']) || isset($_POST['selectSubmit']) || isset($_POST['projectSubmit']) || isset($_POST['joinSubmit'])) {
            handlePOSTRequest();
        } else if (isset($_GET['displayTripRequest']) || isset($_GET['groupByRequest']) || isset($_GET['havingRequest'])  || isset($_GET['nestedAggRequest']) || isset($_GET['divisionRequest']) ) {
            handleGETRequest();
        } 
		?>
	</body>
