<!--Test Oracle file for UBC CPSC304 2018 Winter Term 1
  ...
  background-size: 100% 100%;
    ...
    <body>
        ...
        <h3>  Find the average trip fee of each transport vehicle belonging to at least three trips (nested aggr) </h3>
        <form method="GET" action="trip.php"> <!--refresh page when submitted-->
        ...
        function handleNestedAggRequest() {
            ...
            $result = executePlainSQL("SELECT t.transid, AVG(A.fee)
                                        FROM trip t
                                        GROUP BY t.transid
        ...
        function handleDivisionRequest() {
            ...
            $result = executePlainSQL("SELECT t.origin 
                                       FROM  trip t
                                       WHERE NOT EXISTS ( (SELECT DISTINCT AI.model 
                                                           FROM trip t1, Airplane AI
                                                           WHERE t1.transid = AI.transid AND t1.origin = 'Vancouver')
                                                            MINUS
                                                          (SELECT  AI1.model 
                                                           FROM  Airplane AI1   
                                                           WHERE t.transid = AI1.transid ))" );

            echo "<br>Listing origin names:<br>";
            echo "<table>";
            echo "<tr><th> origin Names </th></tr>";
            while ($row = OCI_Fetch_Array($result, OCI_BOTH)) {
